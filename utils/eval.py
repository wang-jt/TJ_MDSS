...
sys.path.append('/share2/wangyx/MSVD/utils/coco-caption')
# sys.path.append('/share2/wangyx/hope/test/dstc7avsd_eval/utils/coco-caption/pycocotools')
...
def evaluate(ref, exp_id, ref_path='/share2/wangyx/MSVD/inputs/full/ref.json'):
...
def eval(ref, exp_id, ref_path='/share2/wangyx/MSVD/inputs/full/ref.json'):
    swfilter = StopwordFilter('utils/stopwords.txt')
    annos = []
    img_id = 1
    for hyp in ref['dialogs']:
...
if __name__ == '__main__':
    # with open('/share2/wangyx/marlgpt/test/dstc7avsd_eval/sample/baseline_i3d_rgb-i3d_flow.json') as jh:
    #     exp_ref = json.load(jh)
    # evaluate(exp_ref, 'tmp')
    # exp()
    # ckpt_lst = range(10377, 207541, 10377)
    # ckpt_lst = range(41508, 290557, 41508)
    # ckpt_lst = [332064]
    # # eval_batch(ckpt_lst, 'bart_ft_val_sess')
    # eval_batch(ckpt_lst, 'bart_ft_val_wosess')
    # with open('results/barttest/result_beam_search.json') as jh:
    #     test = json.load(jh)
    # evaluate(test, 'test/_bs_beam_search')
    # with open('results/barttest/result_greedy_search.json') as jh:
    #     test = json.load(jh)
    # evaluate(test, 'test/_gs_greedy_search')
